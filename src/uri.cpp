         ...
         string_view path, string_view query, string_view fragment)
{
    _port_i = port;
    std::string port_str;
    if (port > 0) port_str = std::to_string(_port_i);
                      ...
                      string_view path, string_view query, string_view fragment)
{
    ...
    if (!port.empty())
    {
        ...
        it += port.length();
        std::from_chars(_port.data(), _port.data()+_port.size(), _port_i);
    }
    if (!path.empty())
    {
...
{
    uint16_t new_port_i = 0;
    if (auto[p, ec] = std::from_chars(port.data(), port.data()+port.size(), new_port_i); p != port.data()+port.size())
    {
        throw uri_syntax_error{cat("Failed to parse port '", port, "'")};
    }
    _port_i = new_port_i;
...
void URI::set_port(uint16_t port) { if (port <= 0) set_port(""); else set_port(std::to_string(port)); }

void URI::set_path(string_view path)
{
    if (!path.empty() && path.front() != '/') throw uri_syntax_error{cat("Invalid path: '", path, "'")};
...
static bool _tokenize_path(URI::string_view path, std::vector<URI::string_view>& tokens)
{
    ...
    {
        if (tokens[ind] == ".") tokens[ind] = std::string_view{};
        else if (tokens[ind] == "..")
        {
            int back_ind = ind-1;
            while (back_ind >= 0)
            {
                ...
                {
                    if (tokens[back_ind] != "..") tokens[ind] = std::string_view{};
                    break;
                }
                --back_ind;
            }
            tokens[back_ind] = std::string_view{};
        }
    }
    return true;
}
...
{
    std::vector<std::string_view> path_tokens;
...
{
    std::vector<std::string_view> base_tokens;
    std::vector<std::string_view> relative_tokens;
...
static std::string _resolve_path(URI::string_view base, URI::string_view child)
{
    ...
    if (cn == 0)
    {
        ...
        if (i >= 0)
        {
            ...
            return static_cast<bool>(normalized) ? std::move(*normalized) : (std::string) path;
        }
    }
    else
    {
