...
static int_fast16_t letter_to_hex(char in)
{
    ...
    if ((in >= 'A') && (in <= 'F')) return in + 10 - 'A';
    throw uri_syntax_error{cat("Unable to decode character with symbol '", std::string_view{&in, 1}, "'")};
}
...
void URI::_decode_encoded_unreserved_chars()
{
    ...
    while (it != _uri.end())
    {
        if (*it == '%')
        {
            ...
            if (opt_char == '\0')
            {
                throw uri_syntax_error{cat("Unable to decode characters outside the ASCII character set: '",
                                       std::string_view{&*++it, 1}, std::string_view{&*++it, 1}, "'")};
            }
...
inline static bool _advance(std::string_view::const_iterator &it,
                            std::string_view::const_iterator last, uint_fast32_t mask)
{
    bool res = false;
...
static inline bool _skip_pct_encoded(std::string_view::const_iterator &it, std::string_view::const_iterator last)
{
...
static inline bool _skip_pchar(std::string_view::const_iterator &it, std::string_view::const_iterator last)
{
...
static bool _validate_scheme(std::string_view::const_iterator &it, std::string_view::const_iterator last)
{
    return _advance(it, last, T_SCHEME) && *it == ':';
}

static bool _validate_user_info(std::string_view::const_iterator it, std::string_view::const_iterator last)
{
    while (it != last)
    {
...
static bool _validate_fragment(std::string_view::const_iterator &it, std::string_view::const_iterator last)
{
    while (it != last)
    {
...
static inline bool _read_port(const URI::string_view& port_str, uint16_t& port)
{
    ...
    uint32_t int_port = 0;
    if (auto[p, ec] = std::from_chars(port_str.data(), port_str.data() + port_str.size(), int_port);
            p != port_str.data() + port_str.size() || int_port >= std::numeric_limits<unsigned short>::max())
    {
        return false;
    }
...
void URI::_parse(string_view::const_iterator &it, string_view::const_iterator last)
{
    ...
    while (it != last)
    {
        ...
        else if (hp_state == hier_part_state::authority)
        {
            ...
            {
                throw uri_syntax_error{cat("Failed to parse authority info '", _uri, "': ",
                                           std::to_string(first - _uri_view.begin()))};
            }
            ...
            if (*it == '@')
            {
                ...
                {
                    throw uri_syntax_error{cat("Failed to parse user info '", _uri, "': ",
                                               std::to_string(first - _uri_view.begin()))};
                }
            ...
            else if (*it == '/') /* we skipped right past the host and port, and are at the path. */
            {
                ...
                {
                    throw uri_syntax_error{cat("Failed to parse host and port info '", _uri, "': ",
                                               std::to_string(first - _uri_view.begin()))};
                }
        ...
        else if (hp_state == hier_part_state::host)
        {
            if (*first == ':')
            {
                throw uri_syntax_error{cat("Unexpected symbol ':' '", _uri, "': ",
                                           std::to_string(first - _uri_view.begin()))};
            }
            if (*it == ':')
            {
        ...
        else if (hp_state == hier_part_state::host_ipv6)
        {
            if (*first != '[')
            {
                throw uri_syntax_error{cat("Expected symbol '[' in IPv6 portion '", _uri, "': ",
                                           std::to_string(first - _uri_view.begin()))};
            }
            if (*it == ']')
            {
                ...
                if (it == last)
                {
                    ...
                    {
                        throw uri_syntax_error{cat("Failed to set host and port '", _uri, "': ",
                                                   std::to_string(first - _uri_view.begin()))};
                    }
        ...
        else if (hp_state == hier_part_state::port)
        {
            if (*first == '/') /* the port is empty, but valid */
            {
                ...
                {
                    throw uri_syntax_error{cat("Failed to parse port '", _uri, "': ",
                                               std::to_string(first - _uri_view.begin()))};
                }
            ...
            else
            {
                ...
                {
                    throw uri_syntax_error{cat("Expected digit symbol '", _uri, "': ",
                                               std::to_string(it - _uri_view.begin()))};
                }
            }
        }
        ...
        while (it != last)
        {
            if (!_skip_pchar(it, last))
            {
                if (*it !='?' && *it != '/')
                {
                    if (*it != '#')
                    {
                        throw uri_syntax_error{cat("Expected beginning of fragment '", _uri, "': ",
                                                   std::to_string(it - _uri_view.begin()))};
                    }
    ...
    {
        throw uri_syntax_error{cat("Invalid fragment '", _uri, "': ", std::to_string(it-_uri_view.begin()))};
    }
    ...
    if (state == uri_state::hier_part)
    {
        ...
        else if (hp_state == hier_part_state::host_ipv6)
        {
            ...
            throw uri_syntax_error{cat("Invalid IPv6 address '", _uri, "': ",
                                       std::to_string(first - _uri_view.begin()))};
        }
        else if (hp_state == hier_part_state::port)
        {
